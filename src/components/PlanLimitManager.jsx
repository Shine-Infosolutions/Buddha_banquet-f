...
const PlanLimitManager = () => {
  ...
  const PlanEditor = ({ plan, onSave, onCancel, onDelete }) => {
    ...
    useEffect(() => {
      api.get('/api/categories/all')
        .then(res => setEditorCategories(Array.isArray(res.data) ? res.data : []))
    ...
    const handleCreateCategory = async (categoryData) => {
      try {
        ...
        setEditorCategories(Array.isArray(res.data) ? res.data : []);
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          {/* Modal Header */}
          <div className="bg-[#c3ad6b] px-6 py-4 rounded-t-xl">
